...
- **Determinism is a P0 invariant** (`docs/05` §3). One seeded PRNG drives all randomness — hits, status, crits, AI, loot — in a declared roll order. **Never introduce `Math.random`, wall-clock or platform RNG into simulation code.** Rewind, saves and build-sharing depend on it. `npm run check:rng` scans `src/sim` and `src/render/playtest.ts`, but **any module that emits commands is state-bearing**: `src/render/session.ts` produces the command log, so hand-check it. (It is clean — no wall-clock, no timers. Enemy turns run themselves (ADR-0044): entering `AI_TURN` schedules **one** Step through `src/render/pacer.ts`, which owns no clock — its scheduler is injected — and is epoch-guarded, so "how many commands so far" is still never a function of elapsed time. `check:rng` scans both files.)
...
- **A discriminator that separates ABSENT from PRESENT cannot separate RIGHT from WRONG.** The portrait spec asserted `naturalWidth` 192 against the placeholder's 96 — and every one of the six real crops is 192 wide, so a knight's face on the archer passed. When the claim is "the right X", assert X's identity (the asset name in `src`, the key, the id), not only that some X arrived.
