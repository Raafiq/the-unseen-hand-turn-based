...
- **Use the discriminating case.** Inputs where the right behavior gives a *different* answer than the plausible wrong one — never a tie where all orderings coincide. When a change **flips a comparator's primary key**, a fixture that ties on that key is decided by the *old secondary* key and may coincide with the new behavior: make the two keys **disagree**. A doc's AC outranks a specialist's contradicting sub-detail.
...
- **AN ACCEPTANCE CRITERION THAT QUANTIFIES OVER A SET IS SATISFIED BY ITS WEAKEST MEMBER — name the case you mean.** `docs/11` AC-M1 said "the campaign SHALL be driveable to an ending" without naming the player it assumed, so it was met by a party that never spent a point of AP or filled a chassis slot: the customization spine the whole game is built on was optional, and the criterion was green. "X is possible" is a family of claims, and the test you write will pick the easiest member. State which member is the claim (ADR-0027).
