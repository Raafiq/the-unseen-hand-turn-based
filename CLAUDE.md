...
**Status: M0 — the playable slice (`docs/11`), in progress.** A pure headless simulation (`src/sim`) plus a thin viewer (`src/render`), backed by 688 tests and 17 browser specs, a determinism guard, CI, and a GitHub Pages deploy. **A campaign is now playable start to finish** at `/game.html` — title screen, one `localStorage` save slot, five battles, a party that keeps what it earns, scene text before and after each fight, and a prep screen between them (ADR-0022, ADR-0023, ADR-0024). M0 still lacks equipment (item 5) and onboarding (item 7).
