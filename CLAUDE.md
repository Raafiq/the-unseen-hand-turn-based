...
**Status: M0 — the playable slice (`docs/11`), in progress.** A pure headless simulation (`src/sim`) plus a thin viewer (`src/render`), backed by 647 tests and 15 browser specs, a determinism guard, CI, and a GitHub Pages deploy. **A campaign is now playable start to finish** at `/game.html` — title screen, one `localStorage` save slot, five battles, a party that keeps what it earns (ADR-0022, ADR-0023). M0 still lacks the between-battle prep loop, story stubs, equipment and onboarding.
