...
**Status: M0 — the playable slice (`docs/11`) — ALL SEVEN ITEMS BUILT.** A pure headless simulation (`src/sim`) plus a thin viewer (`src/render`), backed by 752 tests and 23 browser specs, a determinism guard, CI, and a GitHub Pages deploy. **A campaign is playable start to finish** at `/game.html` — title screen, one `localStorage` save slot, five battles, a party that keeps what it earns and chooses who deploys, weapons on an authored drip, scene text around each fight, and a prep screen between them (ADR-0022 … ADR-0026). **What is NOT established: that a stranger can play it.** Every automated run drives the balance probe or a deliberate forfeit, so "completable" is reachability — never difficulty, pacing or fun. No one who did not build this has played it.

The **engine** roadmap sits at P2. **Its open exit criterion is the build-diversity gate, at N=7 against a release bar of ≥8** (`docs/06` AC-E2, ADR-0014, ADR-0020) — **carried into M1 by user decision** (`docs/11` §3), not weakened. The viewer's resolution-transparency previews are a P2 deliverable, not P3 — P3 (`docs/08` §1) is hybrid/fusion jobs, rewind UI, scan and the speed toggle, and none of the last three are shipped. The docs in `docs/` are the **source of truth and outrank the code** when the two disagree — a mismatch means the code is wrong, or the doc needs an explicit, recorded change.
...
- **A TEST THAT COVERS A SUBSET READS AS COVERING THE SET.** Not the same failure as the unasserted spec above — here the spec HAS a test, the test passes, and the gap is invisible because nothing states what it skipped. Twice in one slice: the "every job's skillset donates a live-formula action" check listed only the four **P2** jobs, so Knight's `battle-skill` and Thief's `steal` shipped **entirely inert** (every action `formula: "none"`) for the life of the repo; and `ttk.test.ts` held the shipped **builds** to the `docs/07` band while the viewer's hand-authored demo roster — which nothing covered — sat 3–4× outside it just as long, so the playable demo one-shot everything. Both had a green suite the whole time. **When you write or read an assertion over "the content", enumerate what content actually exists**, then either cover all of it or name the exclusion in a manifest the test partitions against exactly (`DEFERRED_SKILLSETS`, `DEFERRED_SUPPORT_EFFECTS`, the gate's `EXCLUDED`) — and verify the partition fails on a STALE entry too, not just a missing one. **The same holds for a reconciliation TABLE in a doc, and there it is harder to see.** `docs/02` B0 exists to catch redundant currencies — its rule is "if any two rows can't be told apart by a player, one is cut or merged" — and it polices only the rows *present*. Character **Level** was never a row, so for the life of the repo it was neither cut nor kept, while B4 and AC-J7 both quietly assumed it existed and drove growth (ADR-0021). A gate that reconciles a domain must be checked against **what the domain contains**, not against what the table lists.
- **AN OUTCOME SET THAT INCLUDES THE FAILURE CASE IS NOT AN ASSERTION — and a discriminator you reasoned about is not a verified one.** Two halves of the same mistake, both earned in one slice. `benchmark-suite.test.ts` asserted every shipped encounter reached a `DECISIVE` outcome — a SET, `{victory, defeat, draw}` — which reads as "the battle works" while in fact every one of them ended in **defeat** for the player side, and an unreachable victory condition would have passed just as green (mutation-checked: point an encounter's `victory` at one of its own team-0 units and the DECISIVE test not only stays green, it reports `victory`, because victory is evaluated before defeat). **Assert the specific outcome you mean**, and if the honest assertion needs a lever — AC-E6 runs the opposition passive — guard that the lever is honored, or an ignored option gives two identical runs. The other half: **when a test comment names the bug it catches, run that mutation.** AC-V13's first draft said asserting `ENDED` right after a killing click discriminates the "banner a turn late" bug. It does not — the check *before* the turn advance already sees that wipe — and the test was green against the very mutant its comment described. Only a KO landing *during* the advance (a charge maturing on the last survivor) reaches the second check. A named discriminator is a claim about code you have not run; break the code and watch the right test go red.
...
- **AN A/B BETWEEN TWO CALLERS OF THE SAME HELPER CANNOT SEE A BUG IN THE HELPER.** Factoring a mechanism into one function is right — one place to get it wrong — but it moves where the evidence has to point, and that is easy to miss. The shell slice put the campaign's whole party-carry into `campaignBattleRecords`, then verified the new player-driven path by byte-comparing its save and report against the headless runner's. Both call it: swap `save.party` for `def.party` and the two paths agree **perfectly**, on the wrong answer — only the older single-path test went red. A comparison between two callers proves they *agree*; it says nothing about what they agree on. Each path still needs one assertion that reaches THROUGH the helper to an observable end (here: write a change via `updatePartyMember`, deploy, and read it off the battle unit). Mutation-verified in both directions.
...
- **An N-bump moves ALL the gate's records together, and a "contained" ability edit often isn't.** When `DIVERSITY_TARGET_N` changes or a build moves EXCLUDED↔MEASURABLE, sync every record in the SAME slice — the `gauntlet.ts` manifest, an ADR-0014 amendment, **`docs/06` AC-E2 (authoritative, outranks the ADR — a review caught it at N=5 while the code said 6)**, **`docs/08` §1a's phase checklist** (authored, not derived — `npm run state` does NOT cover it, and it went two slices stale reading `N = 6` while `DIVERSITY_TARGET_N` said 7), **`docs/11` §3's M0 status table** (the same shape, same decay risk — a second authored "what is done" record now exists, so a slice that finishes an M0 item must move it there too), and a regenerated `npm run state`. **`npm run state` derives its counts from the repo, so it goes stale on ANY commit that adds a counted artifact** — an ADR, an encounter, a build, a spec file — not only on a gate change. Regenerate it as the **last** step of a slice and commit the result: a clean run taken mid-slice proves nothing about the commit you eventually push (adding ADR-0021 moved the count 20 → 21 and failed CI on a branch whose 580 tests were green). **And "derived" does not mean "complete": each counter enumerates a NAMED DIRECTORY**, so a new content directory is invisible to it until wired in — `data/campaign/encounters` shipped 5 battles the encounter count could not see, and the page would have read 6 while 11 existed. A count that cannot see half its domain is the subset-reads-as-the-set failure wearing a derived number's credibility. A **global ability-property edit** (e.g. a shared `summon.*` range) is byte-identical only for *gauntlet substitution* slots — it also changes any **as-authored** encounter fielding a build that learns it, and `benchmark-suite.test.ts` (self-consistency only, no committed golden) will **not** flag the shift; grep the ability across `data/encounters` + `data/builds` and state what moves.
...
- **Story-prose guard:** `npm run check:story` — fails if any test asserts a literal phrase from `data/campaign/story/*.story.json`. The pack is swappable by contract (`docs/11` AC-M4), so a test pinning its prose makes exercising that seam a build failure; four did.
...
- **Everything CI runs:** `npm run check` (typecheck + lint + check:rng + check:handoff + check:story + test)
...
- **A PR body is AUTHORED, and "I pushed the branch" is not a delivery.** The harness forbids opening a PR unasked, so the human often opens it — and GitHub then fills the body from the head commit message: hard-wrapped, no headings, `Co-Authored-By` / `Claude-Session` trailers leaked in. That is what they sit down to review. If you did not open the PR, look for one on your branch and **replace an auto-filled body** (auto-filled iff it equals the head commit message) with the convention: lede, review-artifact link, a `Claim / the bug it hides / caught by` evidence table, what is deliberately **not** asserted, the checks (cf. PR #35, #36). Then re-fetch the stored body to confirm nothing mangled. No hook can catch this — the PR is created outside the session and the sandbox proxy 403s `/repos/{owner}/{repo}`.
