...
> **Status: M0 — all seven items built.** A pure headless simulation (`src/sim`) plus a thin viewer (`src/render`), backed by 994 tests, 196 browser specs, a determinism guard, CI, and a GitHub Pages deploy. The stack is Web / TypeScript (ADR-0007).
>
> **You can play a campaign start to finish** — title screen, New Game / Continue, five battles in order, a scene player that reads story a line at a time beside a portrait frame, a prep screen between battles in two views — party select, then member detail (ADR-0041) — where you spend AP, change job and change loadout, weapons on an authored drip, a **six-member** party that keeps what it earns, and one save slot in the browser (ADR-0022 … ADR-0026, ADR-0029, ADR-0041). Who deploys is authored by the encounter, not chosen. **Hand-play is suspended until the combat revamp** (ADR-0041): wizards and priests have no tap-castable action, so 13 tests are parked. The board **moves** on a commit (ADR-0032) and a **stat panel** sits on the battle map, bottom-left, naming the acting unit (ADR-0033). The campaign is the **site root** (`/`); the engine viewer is `/viewer.html`. ~~What M0 still lacks: equipment and onboarding.~~ Both shipped (ADR-0025, ADR-0026). **What is NOT established: that a stranger can play it** — nobody outside the build has played it, so difficulty, pacing and session length are all unmeasured (`docs/11` §3).
>
