...
- The project's **resolved templates**, read-only, and for one thing only: the section names and section **order** they define, which Step 4's missing-section rule needs. Never take content from a template. Obtain them by running the core resolver rather than by reading `.specify/templates/` directly, because a template is a **stack**, not a file: a project override, then each installed preset in registry-priority order, then any **extension** that ships that template, then the base. Reading the base file alone silently ignores every layer above it. Override and extension layers always **replace**; only a preset may compose (`prepend`, `append`, `wrap`).
  ...
  ```
  Use the sibling matching this project's script variant, the runtime the `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` invocation above already names. **The three filenames do not follow one pattern, so they are given rather than guessed**, and a guessed name fails silently into the fallback below:
  - bash: `.specify/scripts/bash/resolve-template.sh <name> --json`
  - powershell: `.specify/scripts/powershell/resolve-template.ps1 <name> -Json`
  - python: `.specify/scripts/python/resolve_template.py <name> --json`, invoked the same way the `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` line above invokes its own `.py` file (note the **underscore**; only the Python file spells it that way)
...
- `tasks.md` updated with new task IDs allocated by the convention the file already uses, exact file paths, and no `[P]` marker **on the tasks this run added**. Tasks written by `/speckit-tasks` legitimately carry `[P]`; never strip it from them. No task landed under a Convergence phase.
