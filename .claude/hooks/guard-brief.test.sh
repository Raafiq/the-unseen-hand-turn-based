...
t 0 "frame named, LOOK given"           viewer-engineer "Match sheet-B-pass1.png. LOOK: strip < 60% board width, font >= 11px. $ENG"
t 2 "frame named, no LOOK"              viewer-engineer "Match sheet-B-pass1.png. $ENG"
t 0 "frame named, combat-engineer"      combat-engineer "See board.png. $ENG"
echo "-- art director --"
...
t 2 "second spawn, no token"            docs-steward "Rewrite INTENT.md. REPORT in under 10 lines."
t 0 "second spawn with token"           docs-steward "SECOND-PASS-OK: the code changed after the ADR. Rewrite INTENT.md. REPORT in under 10 lines."
echo "-- release engineer, retro before PR --"
export RETRO_MARKER="coverage/agent-spawns/$SID/retro-done"; mkdir -p "$(dirname "$RETRO_MARKER")"
REL="Push the branch and open the PR to main. REPORT in under 10 lines."
t 2 "PR brief, no retro marker"         release-engineer "$REL"
touch -d '5 hours ago' "$RETRO_MARKER"
t 2 "PR brief, stale retro marker"      release-engineer "$REL"
touch "$RETRO_MARKER"
t 0 "PR brief, fresh retro marker"      release-engineer "$REL"
t 0 "'pull request' spelled out, fresh" release-engineer "Open a pull request. REPORT in under 10 lines."
rm -f "$RETRO_MARKER"
