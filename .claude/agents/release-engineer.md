...
- **`docs/INTENT.md`'s stamp names the branch's BASE, never the commit you are making.**
  ...
  not exist. Stamp the base — it is an ancestor, and it is the history the intent doc
  ...
  Confirm with `git show HEAD:docs/INTENT.md`, never with the checker alone.
