...
## 2. ~~The party's only healer cannot heal (viewer, found 2026-09-02)~~ — RETIRED

Fixed by ADR-0043's combat-revamp shell: `isClickTargetable` now accepts `aoe` and `speed`
abilities, so `white-magic.cure` is tap-castable (`docs/INTENT.md`, "Built (M0)"). This
entry was left reading live past that fix; corrected here.
