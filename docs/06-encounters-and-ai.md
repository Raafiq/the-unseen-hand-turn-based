...
> **Implementation status (P2 Slice 3; support-aware update 2026-08-03).** The benchmark suite authors the archetypes expressible today — Skirmish, Siege/choke, Assassinate, Elevation, Anti-cheese, Attrition (`data/encounters/`). Three remain **`[DEFERRED]` on engine capability, not design**: **Defend / survive-N** still needs a `surviveTurns` win condition (the `Condition` union only has `eliminateTeams`/`defeatUnit`); **Escort / protect** still needs support-aware **positioning** (hold-position / protect-the-enabler / body-screen a fragile ally). **Heal-in-encounter partially landed:** the balance-probe now **triages** — it heals the lowest-effective-HP ally (AC-E3(b) mirrored onto allies, ADR-0014 amendment), so a healer with a wounded ally *in range* saves the dying one first. But the probe still has **no hold/protect positioning**, so a fragile ally or squishy healer with nothing in range still advances toward the enemy; Escort and Defend do not unlock until positioning + the `surviveTurns` condition land. Don't re-attempt those two against the current probe.
>
