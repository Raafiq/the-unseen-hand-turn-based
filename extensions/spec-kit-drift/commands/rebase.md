---
description: "After a rebase, detect incoming upstream changes that affect files or names the feature's spec or plan reference, and propose artifact edits"
argument-hint: "[<upstream-ref>] [--since <old-base-ref>]"
scripts:
...
(usually `main`). Your job is to find which of those commits touch something the feature's
`spec.md` or `plan.md` relies on, rank what that breaks, and propose the artifact edits that
put the spec and plan back in line with what upstream now is. **Git is the only detector.
...
error: print `ERROR: Usage: __SPECKIT_COMMAND_DRIFT_REBASE__ [<upstream-ref>] [--since <old-base-ref>]`
...
## Step 3: Extract what the artifacts reference

Read `SPEC` and `PLAN` in full (they are the artifacts being audited; reading them is
never scope creep). Build a **reference table** with one row per reference and where it
appears, named by the citation ladder — an ID (`FR-003`, `SC-001`, `US1/AC2`, `T004`), else
a quoted heading or opening phrase (`"export interface Tag"` for a code block), never a bare
section name:

- **Paths**: any token containing `/` and a file extension, or any path a plan section such
  as Project Structure lists (with or without a trailing description). Keep the path, its
  basename, and its basename without extension.
...
- **Plan claims**: every sentence in `plan.md` that states an obligation or a fact about the
  code — each `TD-###` decision, each `**Constraints**` clause, each Summary sentence that
  says what the code does or never does (`"the save never overrides who deploys"`). A claim
  names no path and no identifier, so it can only be checked against a hunk that is already
  in scope (Step 4); record it so Step 5 can.

Be conservative: a name that appears anywhere in either artifact is referenced.
...
1. `git diff --name-status "$OLD_BASE" "$INCOMING_TIP"` gives every changed file with its
   status (`A`, `M`, `D`, `R…`).
2. A changed file is **in scope** only when `spec.md` or `plan.md` references it by path —
   ...
   `plan.md` lists (architecture, Project Structure, module references), where a `+`/`-`
   line carrying one maps the hunk to the spec item it touches (Step 5). A file `plan.md`
   does not list enters scope only through a `spec.md` path reference, never through a
   name it happens to share.
   Every other changed file is **out of scope**: list it under "Changed, not referenced" in
   the report, take no finding from it, and do not open it.
...
none referenced by spec.md or plan.md**, list the changed files, and stop. Nothing is written.
...
| `restructured` | a file or module the plan names moved, split or was renamed |
| `cosmetic` | naming, import path or formatting only |

| Severity | Rule |
|---|---|
| CRITICAL | `removed` — an export, interface, type or file the spec depends on was changed beyond recognition or deleted |
| HIGH | `signature` or `behaviour` on something the spec describes |
| MEDIUM | `restructured` on something the plan references |
| LOW | `cosmetic` |
...
path the plan lists, say).

**Then check every plan claim against every in-scope hunk.** A hunk that makes a claim
false (`applyDeployment` now substitutes placements; TD-002 says the save never does) is a
`behaviour` finding whose source is that claim (`plan: TD-002`) and which reaches
`plan.md`. Plan claims are the one reference that has no name to match on, so this pass is
what keeps a prose-only decision from slipping through.
...
- **`tasks.md`**: append only work the rebase leaves undone — a decision task for every
  ...
  and an update task when the plan or tasks name a test file for the changed item that the
...
- **Revision note.** `SLUG = rebase-<OLD_BASE7>-<INCOMING_TIP7>`. If `spec.md` or `plan.md`
  already carries `[Drift: <SLUG>]`, amend that entry's `Reason:` and `Items:` lines in
  place rather than adding another. Otherwise append, under a single `## Revisions` heading
  at the bottom of each artifact you edit (create the heading on first use):
