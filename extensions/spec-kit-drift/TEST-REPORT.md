...
**Spec Kit**: 1.0.6 · **Extension**: `drift` 0.3.0 (directory `spec-kit-drift`; the 0.1.0 results are the main tables, the 0.2.0 and 0.3.0 changes and retests are the last two sections) · **Dates**: 2026-09-13 / 14
...
## 0.3.0 — real rebase, orchestration, sync writes plan.md (2026-09-14)

### The real-rebase scenario

Fixture: this repo's own history has a feature branch that had `origin/main` merged into
it on 2026-08-22 (merge `714619b`). Its two parents are a real feature tip and a real main
tip, so rebasing one onto the other is a genuine `git rebase` with genuine feature commits
— no commits were authored for the test.

```bash
git checkout -b drift-rebase-test aa041fc    # feature tip: 3 commits on base d39d6b5
git rebase 8098246                           # main tip: 5 commits on the same base
# → Successfully rebased and updated refs/heads/drift-rebase-test.
# HEAD 62cf0cd · ORIG_HEAD aa041fc · merge-base(HEAD, 8098246) = 8098246
```

Spec `specs/994-retro-skill/` describes `.claude/skills/retrospective/SKILL.md` and
`.claude/settings.json` as they stood at `d39d6b5`; main's five commits rewrote the skill
and added a `PreToolUse` hook (contradicting plan TD-001 "no hook gates PR creation").
FR-004 was planted with no task so analyze would have a CRITICAL to surface (O8).

**What the command computed (O1/O2):** `ORIG_HEAD = aa041fc` (the pre-rebase tip);
old base = `merge-base(ORIG_HEAD, 8098246)` = `d39d6b5`; range `d39d6b5..8098246`,
5 commits — main's (`a29d7c8 c6573f4 11925ae 2139802 8098246`), not the feature's three,
which appear only in the `ORIG_HEAD` sanity check. It printed
`Old base derived from ORIG_HEAD (aa041fc, the pre-rebase tip). If that was not your rebase, re-run with --since.`

### Rebase — O1–O8 and the stale-`ORIG_HEAD` edge case

| # | Test | Verdict | What happened |
|---|---|---|---|
| O1 | After a real rebase, no `--since` | **PASS** | `ORIG_HEAD` path taken; findings D1 CRITICAL `removed` (FR-001's five sections gone), D2 HIGH `behaviour` (FR-003 + `plan: TD-001`, the new hook) |
| O2 | Range is main's commits, not the feature's | **PASS** | see above |
| O3 | Same findings as `--since d39d6b5` | **PASS** | reference run first (declined), then the `ORIG_HEAD` run: identical tables and evidence |
| O4 | Command runs analyze itself after the write | **PASS** | invoked via the Skill tool inside the command; 3 findings printed inline |
| O5 | Command runs converge itself | **PASS** | invoked inside the command; no dispute of the edits; it did **not** re-file the decision task T004 |
| O6 | Appended tasks reported + `/speckit-implement` suggested | **PASS** | T005 (FR-004's missing ledger row) listed with verdict "real remaining work"; final status: `Next: /speckit-implement to complete T004 and T005` |
| O7 | Rebase with no changes to referenced files (feature switched to 996) | **PASS** | `✅ Clean — 5 incoming commits, 70 changed files, none referenced` · no gate · analyze/converge **not** run |
| O8 | analyze CRITICAL after the write | **PASS** | `⚠ WARNING: analyze reports CRITICAL — E1 FR-004 zero coverage`, stated to predate the run, printed before converge ran |
| edge | Stale `ORIG_HEAD` (`git update-ref ORIG_HEAD 56cab48`) | **PASS** | `ERROR: ORIG_HEAD (56cab48) is not this branch's pre-rebase tip — the commits behind it are not the ones on HEAD …` · nothing written |

**A check that could not fail, replaced.** The first draft of the stale-`ORIG_HEAD` guard
was `git merge-base --is-ancestor OLD_BASE NEW_BASE` — but a merge-base is an ancestor of
upstream by definition, so that check passes for every value of `ORIG_HEAD`. It was replaced
before the run by comparing the subjects of `OLD_BASE..ORIG_HEAD` with `NEW_BASE..HEAD`
(a rebase replays the same commits). The edge-case row is that check firing.

### Sync — SP1–SP5

| # | Test | Verdict | What happened |
|---|---|---|---|
| SP1 | Code change breaks a plan constraint (module-level `let foldCount`, incremented in the fold) | **PASS** | D2 HIGH `plan` on the Constraints clause, D3 MEDIUM `plan` on the Summary — both with plan diffs; D1 HIGH `behaviour` on SC-001; D4 LOW `untracked` (`movementFoldCount`, no edit). Gate listed `spec.md, plan.md, tasks.md`; all three written, each with a `[Drift: movement-fold-count]` note |
| SP2 | Code change affects only the spec (absent `move` key now throws) | **PASS** | D1 HIGH `behaviour` FR-002 → spec.md only; gate listed `spec.md` only; no plan diff; `no` → nothing written, Steps 10–12 skipped |
| SP3 | analyze runs inside the command | **PASS** | 5 findings inline incl. a CRITICAL (the mutation breaks the repo constitution's sim-purity rule) and an Overview sentence ("a pure fold") the line-scoped edit left stale — surfaced, with `⚠ WARNING` |
| SP4 | converge runs inside the command, does not fight the plan edit | **PASS** | `tasks_appended`; its `contradicts`/`unrequested` were about the **code** (the mutation, the new export), never about what sync wrote |
| SP5 | Appended tasks reported inline, `/speckit-implement` suggested | **PASS** | T006–T008 listed with verdicts; final status `Next: /speckit-implement …` |

### Orchestration vs running the commands by hand

Same results. In every chained run the agent invoked `/speckit-analyze` and
`/speckit-converge` through the Skill tool (not by hand), against the freshly written
artifacts, and their tables matched what the standalone runs in earlier rounds produced for
the same state: converge accepted the edits, did not re-file tasks the command had already
recorded, and filed only genuinely remaining work. What the chain adds is what a developer
running three commands would otherwise miss: analyze catching the residue a surgical edit
leaves (a stale Overview sentence, a `[X]` task's old wording) in the same output.

### Notes and open items

| Item | Status |
|---|---|
| `[NEEDS REVISION]` marker text named the file (`SKILL.md removed`) when only the FR's sections were removed | **Fixed** — the template now names the thing that is gone; not re-run |
| FR-002 of the retro spec ("the skill only proposes, never auto-applies") was not flagged although main reworded that passage | **Unverified** — one run; the agent may have judged the new text still satisfies it. Not checked by hand |
| Every O/SP row is one run of an agent following the prompt | as before — evidence, not a guarantee |
| The rebase replay happened to be conflict-free; a conflicted rebase (resolved by hand) still leaves `ORIG_HEAD` at the old tip, so the path is the same, but that case was not run | untested |

### Ship

**Ship.** 0.3.0: every O and SP row passes; the one prompt-side defect found on the way
(the vacuous ancestor check) was replaced and the replacement is the row that fired.
