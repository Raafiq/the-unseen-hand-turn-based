...
const PATHS: Record<string, string> = {
  ...
  /**
   * The Primary skill slot's padlock, drawn in the chevron's own position. "LOCKED TO
   * JOB" is ~50px of chip and the dossier's skill sub-column gives the value ~120px, so
   * the words ellipsised the ability name; the mark plus a `title` keeps the row's
   * roundel / label / value / affordance shape. Lifted verbatim from the approved
   * mockup (`mockups/src/icons.js`).
   */
  lock: '<rect x="5.4" y="10.6" width="13.2" height="10.4" rx="1.6"/><path d="M8.4 10.6V7.8a3.6 3.6 0 0 1 7.2 0v2.8"/>',
  /**
   * The Worn Armor slot's own mark — a cuirass in the same 1.8-weight open-stroke style
   * as every other glyph here, so the empty slot reads as a slot rather than as a note.
   * Shoulder line, breastplate, centre seam. Mockup pass 13, verbatim.
   */
  cuirass:
...
export function icon(id: string, extraClass = ""): string {
  ...
  // `data-icon` is the glyph's IDENTITY on the page. Without it a spec can only assert
  // that *an* `<svg>` arrived, which cannot tell the cuirass from the sword — the same
  // "absent vs present cannot judge right vs wrong" trap the portrait width assertion
  // fell into (root CLAUDE.md). Nothing styles off it; it exists to be asserted.
  return `<svg class="${cls}" data-icon="${id}" aria-hidden="true" ${attrs}>${paths}</svg>`;
}
